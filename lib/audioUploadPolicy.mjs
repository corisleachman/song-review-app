...
export const AUDIO_UPLOAD_ACCEPT = '.mp3,.wav,.m4a,.aac,.flac,.ogg';
export const AIFF_UPLOAD_UNAVAILABLE_MESSAGE = 'AIFF uploads are temporarily unavailable. Export your audio as WAV or MP3 instead.';
...
const AUDIO_FILE_TYPES = Object.freeze({
  ...
  // Retain inspection of previously allocated/stored AIFF files. New allocations
  // are blocked below until cross-browser playback is supported and verified.
  aif: {
...
export function validateAudioUploadMetadata({ fileName, fileSize, contentType }) {
  ...
  if (normalizedFile.extension === 'aif' || normalizedFile.extension === 'aiff') {
    return { ok: false, reason: 'aiff_unavailable' };
  }
