...
// Shared audio engine for the public playlist players (immersive /listen and
// embeddable /embed). Both surfaces render different UI but drive the exact same
// engine: destroy+recreate WaveSurfer per track, desktop live Web Audio /
// mobile OfflineAudioContext precompute for the reactive EQ, and Media Session.
// Keep this as the single source of truth — do not fork per surface.

import { useCallback, useEffect, useRef, useState, type RefObject } from 'react';

/* eslint-disable @typescript-eslint/no-explicit-any */

// ── Colour + frequency helpers ──
...
/** Format seconds as m:ss. Exported for use in player UI. */
export function fmt(s: number) { if (!isFinite(s) || s < 0) return '0:00'; const m = Math.floor(s / 60); return `${m}:${String(Math.floor(s % 60)).padStart(2, '0')}`; }

export type PlaylistTrack = { id: string; title: string; image_url: string | null; audioUrl: string | null };
export type PlaylistData = { playlist: { id: string; title: string; workspace_name: string | null }; tracks: PlaylistTrack[] };
...
export type PlaylistPlayer = {
  ...
  waveHostRef: RefObject<HTMLDivElement | null>;
  eqCanvasRef: RefObject<HTMLCanvasElement | null>;
  handlePlay: () => void;
  goNext: () => void;
  goPrev: () => void;
...
export function usePlaylistPlayer(id: string): PlaylistPlayer {
  ...
  useEffect(() => {
    ...
    fetch(`/api/public/playlist/${id}`, { cache: 'no-store' })
      ...
      .then((d: PlaylistData | null) => { if (!active) return; if (d) { setData(d); tracksRef.current = d.tracks; } else setMissing(true); })
  ...
  return {
    data, loading, missing, tracks, current, cur, isPlaying, currentTime, duration, palette,
