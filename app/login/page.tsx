...
function LoginContent() {
  ...
  const [emailFormOpen, setEmailFormOpen] = useState(searchParams.get('email') === '1');
      ...
      <div className={styles.page}>
        ...
        <div className={styles.left}>
          ...
          <div className={styles.authPanel}>
            ...
            {emailAuthEnabled && (
              <button
                className={styles.btnEmailChoice}
                type="button"
                aria-expanded={emailFormOpen}
                aria-controls={emailFormOpen ? 'email-auth-form' : undefined}
                onClick={() => {
              ...
              >
                {emailFormOpen ? 'Hide email form' : 'Log in and sign up with email'}
              </button>
            )}

            {emailAuthEnabled && emailFormOpen && (
              <form id="email-auth-form" className={styles.emailForm} onSubmit={handleEmailSubmit} noValidate>
                {isSignup && (
                ...
                <span className={styles.securityCheckLabel}>Security check</span>
                <TurnstileWidget
                  ref={turnstileRef}
                ...
                <div className={styles.modeLinks}>
                  {isSignup ? (
                    <Link href={`/login?email=1${destination === '/dashboard' ? '' : `&redirectTo=${encodeURIComponent(destination)}`}`}>Already have an account? Log in</Link>
                  ) : (
                    <Link href="/login?signupPlan=free&email=1">New to The Song Room? Create account</Link>
                  )}
                </div>
              </form>
            )}
