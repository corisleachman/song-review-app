...
- That result proved the visible percentage can round to 100% without the browser emitting the exact byte-count or upload-load event used by the first grace trigger. The shared uploader now starts and refreshes its 15-second response timer when the same progress calculation shown to the user reaches 100%.
