...
- That result proved the visible percentage can round to 100% without the browser emitting the exact byte-count or upload-load event used by the first grace trigger. The next Preview build used the displayed percentage, but repeated completion-level progress events kept refreshing its timer and the valid AIFF again remained at 100% after 17 seconds.
