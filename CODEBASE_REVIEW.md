...
- UPLOAD-002: Production follow-up in progress. PR #48 merged as `6f655f23`, production migration `20260903163658` applied, and primary deployment `dpl_3sL1SuU2q7LvzENQ4Li9JWmaBSzQ` reached Ready. Live size rejection and renamed non-audio cleanup passed. PR #50 fixes signature-read lifetime handling after successful uploads stalled during finalization. At `56896ca9`, the user's 623,130-byte AIFF finalized on Preview with 200 four seconds after allocation; matching Storage metadata and subsequent version/page reads were verified. Playback then failed with a generic waveform retry message. AIFF/AIF playback support is deferred to P3, and the local candidate blocks new AIFF/AIF uploads while retaining historical inspection. The failure is not proven exclusive to AIFF; MP3/WAV playback and new AIFF rejection remain required Preview checks. PR #50 stays draft and production is unchanged.
