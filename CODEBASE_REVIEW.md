...
- CONFIG-001: Production complete. PR #47 squash-merged into `clone-clean` as `7e4faacd` after the enforced Preview policy passed the public, authenticated, billing-cancellation, framing, and console checks. Primary production deployment `dpl_Fw7k7gH1UrK9Z18EtHJKpi3BsLst` and secondary deployment `dpl_85UaWqDEFCEmxUGmhVM2QbNL6EMB` reached Ready. Live standard responses contain the enforced policy, production Supabase origin, `frame-ancestors 'none'`, and `X-Frame-Options: DENY`; `/embed/*` keeps `frame-ancestors *` without `X-Frame-Options`. The production runtime scan found no application error, 5xx response, or genuine CSP report.
...
- PLAYBACK-001: First-play reliability in the authenticated song/version journey. Following PR #50 Production rollout, the user confirmed AIFF rejection and MP3 upload, but the first Play press did nothing; after leaving and returning, Play loaded the MP3. This is user-observed Production evidence, not independent browser reproduction or proof of a general format failure. Source inspection shows an enabled Play button silently returns before WaveSurfer exists, a timeout starts before lazy audio loading, and automatic retry resets its allowance without resuming user-requested loading. A controlled local harness now executes the actual initialization callback, Play handler, retry callback and lifecycle effect with mocked media, timers and a delayed module import. It reproduces lost early Play presses, successful first Play after initialization, idle timeout/retry-budget reset and lost requested loading after retry. These lifecycle defects are confirmed under controlled conditions, but the cause of this specific Production occurrence remains unconfirmed. Earlier slow-network Preview playback recovered on the same file. The user identified the affected Production version as `c8b3db65-6c2a-4102-ac6e-16784bb6719b` on song `48da7337-f2fc-4df4-93e3-d4633aceae30`, using desktop Chrome, and pressed Play once the button was visible; elapsed initialization time wasn't measured. A subsequent MP3 version played on its first press immediately after upload, after approximately three to four seconds of loading. This narrows the report to intermittent first-play behaviour, not a consistent upload or format failure. The focused lifecycle fix is implemented on `codex/player-first-play-readiness`, based on Production `6a09241e` and published in draft PR #52 into `clone-clean`. It preserves early Play intent, starts deadlines during actual loads, bounds and resumes retries, guards stale callbacks and keeps failures visible. All 69 local tests passed, including 22 new player-lifecycle cases; TypeScript, focused ESLint and an optimized build with build-only placeholders passed. Implementation commit `fae8ee0d` is pushed; Git-triggered Preview checks are pending at publication. Production hasn't changed, and merge/Production rollout isn't authorised. The user reported on 19 September that all requested authenticated Preview checks passed and authorised continuation. Record the final publication-note check set before merge; Production verification remains required.
...
- UPLOAD-002: Production follow-up in progress. PR #48 merged as `6f655f23`, production migration `20260903163658` applied, and primary deployment `dpl_3sL1SuU2q7LvzENQ4Li9JWmaBSzQ` reached Ready. Live size rejection and renamed non-audio cleanup passed. PR #50's finalization fix passed subsequent AIFF and MP3 uploads in Preview. The user confirmed MP3 playback on `aea04a0f` after connectivity recovered, then confirmed successful WAV upload and playback. AIFF playback support remains deferred as P3 and new AIFF/AIF uploads are blocked in the deployed policy, while historical inspection is retained. The user confirmed seeing the AIFF rejection message and approved rollout on 18 September. PR #50 squash-merged into `clone-clean` as `6a09241ee4bcf8d0569d65f4645ad16e3e633248`; primary Production `dpl_CGr85ktfsnHnR5syLVQnngEVDobL` is Ready and serves the live domain. Public route/header and scoped runtime checks passed. The user confirmed Production AIFF rejection and MP3 upload, but the first Play press did nothing; after leaving and returning, Play loaded the same MP3. The user subsequently confirmed another MP3 played on its first press immediately after upload, following a three-to-four-second load. Upload/rejection and a fresh first-press playback smoke test passed by user observation, but the earlier intermittent failure remains unresolved. Track PLAYBACK-001 as a P1 beta journey issue and don't mark the full journey production complete. Legacy project deployment `dpl_DKLwerEnw6hN5uF7WTEo3hayiSDy` is Ready but classified Preview, not Production; no configuration was changed.
  - Full invocation logs on 17 September correct the earlier CLI summary: signed download headers were received. At `856c31fe`, Ready Preview `dpl_2UcJNc5Jr8yPbPvScXyEnMD94MGc`, native HTTPS read 206 and 65,536 bytes, then framework fetch reached `signature_headers_received` and `signature_body_started`. It never finished body reading; requests timed out with 504 after 300 seconds. Next's dedupe wrapper tees responses, and a local regression reproduces one-branch cancellation waiting for its cached sibling. The focused fix supplies an explicit signal (dedupe opt-out), a ten-second deadline, and non-blocking cancellation; the temporary transport comparison is removed. All security checks remain. Local tests pass 47/47, TypeScript and focused lint pass; subsequent Preview uploads, MP3/WAV playback and AIFF rejection passed. Production rollout is approved; pending test data remains untouched.
...
- CSP enforcement merged through PR #47 as `7e4faacd`. Both production deployments reached Ready, live standard and embed framing headers retained their route-specific contracts, and the runtime scan found no application errors or genuine CSP reports.
...
- Focused upload, accessibility, mobile interface, browser-test, homepage-contrast, and CSP follow-ups: production complete through PRs #15, #17 to #26, #28, #29, and #47. UPLOAD-002 reached production through PR #48; PR #50's finalization fix passed AIFF and MP3 uploads in Preview, and the user confirmed MP3 playback after connectivity recovered. The later Production smoke test promotes PLAYBACK-001 to P1 first-play reliability, not blanket format incompatibility. WAV upload/playback and AIFF rejection also passed by user observation. PR #50 is deployed to primary Production as `6a09241e`. Public routes, production-origin CSP, framing and early runtime checks passed; the user confirmed live AIFF rejection and MP3 upload/playback after navigating away and back, but first-click playback failed. A subsequent MP3 upload passed first-press playback after a three-to-four-second load in desktop Chrome. The upload/rejection rollout checks have passed by user observation; intermittent player reliability remains open and full journey production-complete status stays withheld.
