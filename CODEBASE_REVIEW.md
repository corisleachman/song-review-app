...
- AUTH-001: Email/password readiness is a complete user journey and security-boundary change, not a form-only addition. Slice 1 reached Production through PR #54 as `fddab843`: Production middleware no longer trusts `song_review_auth` plus `song_review_identity`; protected requests validate signed Supabase claims; auth destinations share one strict allowlist; and the one-hour AES-256-GCM intent supports email binding and fails closed. Slice 2B is committed and pushed in draft PR #55. All four checks and the hosted-key default-off Preview gate pass on primary deployment `dpl_6aBS3Z21Radpg4MULqau2h1tGdjU`. Login and tier-aware signup controls remain default-off; server routes validate bounded input, reject cross-origin requests, keep provider errors out of public responses, bind continuation to the normalized email, retain referral context, and fail if signup unexpectedly returns a session while verification should be required. `/auth/check-email` is public, no-store, reload-safe in the same tab, and offers a visible resend cooldown plus Google fallback. The staging-only Cloudflare widget is created, its secret is stored in staging Supabase Auth with CAPTCHA enabled, and the public site key is scoped to the PR's Vercel Preview branch. The live Preview CSP admits the Cloudflare challenge origin on standard routes, retains the staging Supabase origin and framing protections, and leaves `/embed/*` unchanged. Every password route requires the bounded token, Supabase Auth receives it for hosted verification, and tokens reset after each attempt. Production remains Google-only with Email disabled. Custom SMTP, the confirmation template, staging URL changes, stronger hosted password settings, a staging auth-intent secret, and the Preview feature flag remain mandatory before enablement. Recovery and account settings remain Slice 3.
- PLAYBACK-001: Production complete. PR #52 squash-merged into `clone-clean` as `9c483ee1` after all requested authenticated Preview checks passed. The fix preserves an early Play request through lazy player initialization, starts deadlines only for real loads, bounds retries, resumes requested loading after a retry, and ignores stale callbacks. Primary Production `dpl_2SyH8hq57prmZeAdfYt4V7oM7Gf3` reached Ready; public route, security-header, and deployment-scoped runtime checks passed. The user then confirmed first-press playback on the live authenticated journey on 19 September 2026. Rollback is a revert of `9c483ee1` or restoration of preceding primary deployment `dpl_CGr85ktfsnHnR5syLVQnngEVDobL`.
...
- UPLOAD-002: Production complete. PR #48 merged as `6f655f23`, migration `20260903163658` was applied, and PR #50 later squash-merged as `6a09241e` to fix upload finalization. Live size rejection, renamed non-audio cleanup, MP3 upload/playback, WAV upload/playback, and AIFF rejection passed. AIFF playback support remains deferred as P3 and new AIFF/AIF uploads stay blocked while historical inspection remains available. The intermittent first-Play issue observed during rollout was handled separately by PLAYBACK-001 and closed through PR #52. Legacy project deployment `dpl_DKLwerEnw6hN5uF7WTEo3hayiSDy` was Preview, not Production; no configuration was changed.
...
- Focused upload, accessibility, mobile interface, browser-test, homepage-contrast, CSP, playback, and tablet-homepage follow-ups are production complete through PRs #15, #17 to #29, #47, #48, #50, #52, and #53. New AIFF/AIF uploads remain intentionally blocked; MP3/WAV upload and playback passed. The remaining physical 11-inch iPad check is post-rollout device coverage, not an unverified Production claim.
