...
- CONFIG-001: Preview user journeys passed on `codex/csp-report-only`. Runtime observation found expected preview-only Vercel Toolbar reports plus one legitimate Google Tag Manager image beacon missing from `img-src`; the local policy now includes that image source and needs one revised preview observation before merge. A route-aware `Content-Security-Policy-Report-Only` header inventories the current Supabase, Google Analytics, Google font, Google avatar, and GitHub-hosted media sources without blocking them. The existing enforced frame policy remains unchanged, including the intentional `/embed/*` exception. Small same-origin violation reports are accepted at `/api/csp-report`, stripped of query strings, hashes, policy text, and script samples, and written only to Vercel runtime logs. Enforcement remains a separate change after report tuning.
