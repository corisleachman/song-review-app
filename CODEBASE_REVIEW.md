...
- UPLOAD-002: Production follow-up in progress. PR #48 merged as `6f655f23`, production migration `20260903163658` applied, and primary deployment `dpl_3sL1SuU2q7LvzENQ4Li9JWmaBSzQ` reached Ready. Live size rejection and renamed non-audio cleanup passed without a 5xx or error-level runtime event. A valid 475,278-byte AIFF then reached 100% and appeared correctly in Storage, but the direct PUT never completed in the browser, so finalization was not called and the version remained pending. PR #50's first Preview candidate reproduced the stall with a valid 475,278-byte AIFF: `/api/versions/create` returned 201, the interface showed 100%, and no finalization request followed after 17 seconds. That proved the browser can round progress to 100% without emitting the exact byte-count or upload-load event used by the first recovery trigger. The second Preview candidate loaded the rounded-progress trigger but remained at 100%, showing that repeated completion-level progress events can keep refreshing the timer while the PUT stays open. The revised shared candidate arms the 15-second grace period once on the first displayed 100% event, then hands control to the authoritative server finalizer. Production is not considered complete until the revised recovery passes in staging and production.
