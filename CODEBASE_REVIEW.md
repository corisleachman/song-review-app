...
- UPLOAD-002: Production follow-up in progress. PR #48 merged as `6f655f23`, production migration `20260903163658` applied, and primary deployment `dpl_3sL1SuU2q7LvzENQ4Li9JWmaBSzQ` reached Ready. Live size rejection and renamed non-audio cleanup passed without a 5xx or error-level runtime event. A valid 475,278-byte AIFF then reached 100% and appeared correctly in Storage, but the direct PUT never completed in the browser, so finalization was not called and the version remained pending. The stop condition exposed that all three clients wait indefinitely when Storage persists an object but leaves its response open. A focused local candidate on `codex/audio-upload-completion` now shares a 15-second post-transfer response grace period, then hands control to the existing authoritative server finalizer. Production is not considered complete until that recovery passes in staging and production.
