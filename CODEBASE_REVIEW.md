...
- CONFIG-001: Local code is complete on `codex/csp-report-only`, pending publication and preview observation. A route-aware `Content-Security-Policy-Report-Only` header now inventories the current Supabase, Google Analytics, Google font, Google avatar, and GitHub-hosted media sources without blocking them. The existing enforced frame policy remains unchanged, including the intentional `/embed/*` exception. Small same-origin violation reports are accepted at `/api/csp-report`, stripped of query strings, hashes, policy text, and script samples, and written only to Vercel runtime logs. Enforcement remains a separate change after preview and production report tuning.
...
- Focused upload, accessibility, mobile interface, browser-test, and homepage-contrast follow-ups: production complete through PRs #15, #17 to #26, #28, and #29. CONFIG-001 is code complete locally in report-only mode and still needs preview observation before any enforcement decision. No confirmed P1 issue remains from this review.
