...
- UPLOAD-002: Production follow-up in progress. PR #48 merged as `6f655f23`, production migration `20260903163658` applied, and primary deployment `dpl_3sL1SuU2q7LvzENQ4Li9JWmaBSzQ` reached Ready. Live size rejection and renamed non-audio cleanup passed without a 5xx or error-level runtime event. A valid 475,278-byte AIFF then reached 100% and appeared correctly in Storage, while the interface remained pending. PR #50's first, second, and one-shot timer candidates reproduced that visible stall. Rounded completion and repeated progress events were working hypotheses, not captured event telemetry. On 17 September, readable diagnostics at `f98deed0` on Ready Preview `dpl_BBP3HxbWRkJ5QAHrWG5dL4z72MW7` proved Storage returned 200 after 923ms and the client requested finalization immediately, without receiving its response. For that test, the broken boundary is finalization, not direct upload completion. Opt-in `?uploadDebug=1` client diagnostics and explicitly requested Preview-only server stage markers now trace the boundary without logging URLs, tokens, filenames, or file content. Missing completed-request log entries do not prove a request was never sent. Identify the pending finalizer operation before another behaviour change. PR #50 remains draft and production is unchanged.
  - Full invocation logs on 17 September correct the earlier CLI summary: signed download headers were received. At `856c31fe`, Ready Preview `dpl_2UcJNc5Jr8yPbPvScXyEnMD94MGc`, native HTTPS read 206 and 65,536 bytes, then framework fetch reached `signature_headers_received` and `signature_body_started`. It never finished body reading; requests timed out with 504 after 300 seconds. Next's dedupe wrapper tees responses, and a local regression reproduces one-branch cancellation waiting for its cached sibling. The focused fix supplies an explicit signal (dedupe opt-out), a ten-second deadline, and non-blocking cancellation; the temporary transport comparison is removed. All security checks remain. Local tests pass 47/47, TypeScript and focused lint pass; Preview verification remains pending. Production and pending test data are untouched.
...
- Focused upload, accessibility, mobile interface, browser-test, homepage-contrast, and CSP follow-ups: production complete through PRs #15, #17 to #26, #28, #29, and #47. UPLOAD-002 reached production through PR #48 but remains open while its direct-upload completion recovery is verified. No confirmed P1 issue remains from this review.
