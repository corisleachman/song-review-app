...
- UPLOAD-002: Production follow-up in progress. PR #48 merged as `6f655f23`, production migration `20260903163658` applied, and primary deployment `dpl_3sL1SuU2q7LvzENQ4Li9JWmaBSzQ` reached Ready. Live size rejection and renamed non-audio cleanup passed without a 5xx or error-level runtime event. A valid 475,278-byte AIFF then reached 100% and appeared correctly in Storage, but the direct PUT never completed in the browser, so finalization was not called and the version remained pending. PR #50's first and second Preview candidates reproduced the visible stall. Rounded completion and repeated progress events were working hypotheses, not captured event telemetry. On 17 September, the one-shot timer candidate at `c3e0f2e9` also remained at 100% on primary Preview `dpl_B6wGmjj1J32y8EAAgNmNhWmrPhtD`; allocation returned 201 and no finalization request appeared. The loaded page asset was `page-dbca185b7d8cb777.js`. Opt-in `?uploadDebug=1` diagnostics now trace the shared uploader's response grace and request events without logging URLs, tokens, filenames, or file content. Establish the failing boundary before another recovery change. PR #50 remains draft and production is unchanged.
