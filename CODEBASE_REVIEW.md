...
- AUTH-001: Email/password readiness is a complete user-journey and security-boundary change, not a form-only addition. Slice 1 is code-complete in draft PR #54 and its full Preview gate has passed: Production middleware no longer trusts `song_review_auth` plus `song_review_identity`; the browser-test exception requires an explicit non-Production switch; protected requests validate signed Supabase claims; callback, Login, middleware, bootstrap, and continuation destinations share one strict route allowlist; and raw Google provider errors no longer cross the callback query string. The new AES-256-GCM intent is versioned, expires after one hour, supports email binding, and fails closed on missing secrets, bad keys, tampering, or expiry. `/auth/confirm` and `/auth/continue` now exist behind the default-off Email boundary, while invite and recovery continuations avoid direct-account bootstrap. Primary Preview `dpl_FRi2S1GDVmFvbAATCoVEPKA6aYZL` returned the expected public and signed-out responses, rejected legacy-cookie bypass, kept Email unavailable, used staging Supabase in CSP, and produced no 5xx or runtime errors. Final documentation-head Preview `dpl_D2b43o3ME9ydGU3jdgv1xLQf21w2` passed all four checks, and the user confirmed the protected Google login, return destination, and persisted session. The current interface remains Google-only and the established Google callback handoff is unchanged. PR #54 remains draft pending code review and explicit rollout approval. `/auth/reset-password` is still a dormant partial flow, and `/api/auth/verify-password` remains an unrelated shared beta-password check. The approved safeguard keeps Production Email disabled; staging Email remains available for controlled development. Forms, SMTP, templates, CAPTCHA, stronger password policy, and hosted rollout remain outstanding.
