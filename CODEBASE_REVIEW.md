...
- UPLOAD-001: Code complete on the preview branch. Playlist-cover upload now rejects oversized requests before multipart parsing, rejects empty, over-5 MB, and non-JPEG/PNG/WebP files before buffering, and caps Sharp decoding at 40 megapixels. A normal JPEG persisted after refresh and appeared on the shared preview. The oversized-file rejection worked, but the client initially showed the local thumbnail and a discreet generic error. The follow-up validates before upload, never previews an unaccepted file, preserves the saved cover, and presents a focus-managed dialog with a specific recovery action. Final preview recheck remains pending.
