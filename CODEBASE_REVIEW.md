...
- AUTH-001: Email/password readiness is a complete user journey and security-boundary change, not a form-only addition. Slice 1 reached Production through PR #54 as `fddab843`: Production middleware no longer trusts `song_review_auth` plus `song_review_identity`; protected requests validate signed Supabase claims; auth destinations share one strict allowlist; and the one-hour AES-256-GCM intent supports email binding and fails closed. Slice 2A is committed as `c4b08016` in draft PR #55. Login and tier-aware signup controls remain default-off; server routes validate bounded input, reject cross-origin requests, keep provider errors out of public responses, bind continuation to the normalized email, retain referral context, and fail if signup unexpectedly returns a session while verification should be required. `/auth/check-email` is public, no-store, reload-safe in the same tab, and offers a visible resend cooldown plus Google fallback. Local TypeScript, focused lint, 80 contract tests, and desktop/mobile render checks passed. All four PR checks passed, and primary Preview `dpl_4QFuynYcjNPN86xdmVwf9pKMpMpi` passed the default-off live gate: Login stayed Google-only, Email routes failed closed, signed-out redirects and staging CSP were preserved, and deployment logs showed no application error or 5xx response. The only CSP reports were for Vercel's injected Preview Toolbar at `vercel.live`. Production remains Google-only with Email disabled. Custom SMTP, the confirmation template, staging URL changes, stronger hosted password settings, and a selected CAPTCHA widget remain mandatory before Preview enablement. Recovery and account settings remain Slice 3.
