...
- AUTH-001: Email/password readiness is a complete user journey and security-boundary change, not a form-only addition. Slice 1 reached Production through PR #54 as `fddab843`: Production middleware no longer trusts `song_review_auth` plus `song_review_identity`; protected requests validate signed Supabase claims; auth destinations share one strict allowlist; and the one-hour AES-256-GCM intent supports email binding and fails closed. Slice 2A is committed in draft PR #55 and passed its default-off Preview gate. Login and tier-aware signup controls remain default-off; server routes validate bounded input, reject cross-origin requests, keep provider errors out of public responses, bind continuation to the normalized email, retain referral context, and fail if signup unexpectedly returns a session while verification should be required. `/auth/check-email` is public, no-store, reload-safe in the same tab, and offers a visible resend cooldown plus Google fallback. Cloudflare Turnstile is implemented locally for Slice 2B. Readiness requires its public site key, every password route requires the bounded token, Supabase Auth receives it for hosted verification, tokens reset after each attempt, and the exact Cloudflare script/frame origin enters CSP only when configured and never on `/embed/*`. Local TypeScript, focused lint, 82 contract tests, and enabled desktop/mobile render checks pass. Production remains Google-only with Email disabled. Hosted Turnstile keys, custom SMTP, the confirmation template, staging URL changes, and stronger hosted password settings remain mandatory before Preview enablement. Recovery and account settings remain Slice 3.
