...
- CONFIG-001: Enforcement candidate prepared locally on `codex/csp-enforcement-readiness`. The report-only policy has been live since PR #30; the available seven-day production logs contain no CSP warnings, a fresh live crawl of the marketing, login, legal, blog, invite, public-listen, and embed surfaces found no policy violations, and the source audit found no new browser origin since the allowlist was tuned. The candidate changes only the route-aware header from reporting to enforcement, keeps the bounded sanitized report endpoint, preserves `frame-ancestors *` for `/embed/*`, and passes the enforced local browser suite. Preview verification of authenticated Google, dashboard, upload, playback, sharing, embedding, and Stripe-cancellation flows remains the release gate.
