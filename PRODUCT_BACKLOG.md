...
- Verify audio upload restrictions (allowed MIME types + max size caps). PR #48 merged as `6f655f23` and migration `20260903163658` is applied to staging and production. MP3, WAV, and M4A passed in Preview; live oversized and renamed non-audio rejection also passed. Production closeout is paused because a valid AIFF reached Storage but its direct PUT response remained open, leaving the browser at 100% before finalization. A focused local follow-up now gives Storage 15 seconds after the last byte, then continues to the authoritative finalizer rather than waiting forever. Verify that recovery in staging and production before marking this item done.
