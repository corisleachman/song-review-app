...
- Verify audio upload restrictions (allowed MIME types + max size caps). PR #48 merged as `6f655f23` and migration `20260903163658` is applied to staging and production. MP3, WAV, and M4A passed in Preview; live oversized and renamed non-audio rejection also passed. Production closeout is paused because a valid AIFF reached Storage but its direct PUT response remained open, leaving the browser at 100% before finalization. PR #50's first Preview candidate confirmed that rounded 100% progress can appear without the exact byte-count or upload-load event that armed its grace timer. The revised candidate now starts and refreshes that timer when the displayed progress calculation reaches 100%, while the server finalizer still proves the object is complete and valid. Verify the revised recovery in staging and production before marking this item done.
