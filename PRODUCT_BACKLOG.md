...
- Verify audio upload restrictions (allowed MIME types + max size caps). PR #48 merged as `6f655f23` and migration `20260903163658` is applied to staging and production. MP3, WAV, and M4A passed in Preview; live oversized and renamed non-audio rejection also passed. Production closeout is paused because a valid AIFF reached Storage while the interface remained at 100%. All three PR #50 recovery candidates reproduced the stall. Readable Preview diagnostics on 17 September proved Storage returned 200 after 923ms and finalization was requested immediately, without receiving its response. Trace the pending finalizer operation using Preview-only stage markers before changing behaviour again. Verify recovery in staging and production before marking this item done.
