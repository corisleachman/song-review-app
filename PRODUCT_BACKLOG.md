...
- Logged: 2026-09-18
- Status: Slice 1 reached Production through PR #54 as merge commit `fddab843`; its live Google-only boundary and deployment passed verification. Slice 2A is committed in draft PR #55 and passed its default-off Preview gate. Cloudflare Turnstile was approved for Slice 2B and is implemented locally: Email readiness requires its public site key, login/signup/resend require a token, Supabase Auth receives it for hosted validation, tokens reset after each attempt, and CSP access is exact and conditional. Production remains Google-only with its Email provider disabled. Staging Email remains enabled, but Turnstile keys are not configured and the built-in trial sender, default templates, stale Site URL, and weak default password settings still block enabling the new UI. The next gate is custom SMTP and controlled staging-only configuration. No hosted setting, email, user, database, billing, or Production behavior changed in Slice 2B.
...
- Staging template contract, configuration order, verification, and rollback: `EMAIL_PASSWORD_STAGING_ROLLOUT.md`.
...
- Add bot protection on signup. Cloudflare Turnstile was selected for Slice 2B and the default-off application integration is implemented locally; hosted staging keys and Supabase Auth enablement remain pending.
