...
- Logged: 2026-09-18
- Status: Slice 1 reached Production through PR #54 as merge commit `fddab843`; its live Google-only boundary and deployment passed verification. Slice 2B is committed and pushed in draft PR #55, all four checks pass, and its default-off Preview gate passed. The staging-only Cloudflare widget is created, its secret is stored in staging Supabase Auth with CAPTCHA enabled, and its public site key is scoped to the PR's Vercel Preview branch. A fresh deployment is pending. The built-in trial sender, default templates, stale Site URL, weak default password settings, missing auth-intent secret, and disabled feature flag still block the new UI. Production remains Google-only and unchanged.
...
- Add bot protection on signup. Cloudflare Turnstile is implemented in draft PR #55; the staging widget, Supabase Auth secret, CAPTCHA setting, and branch-scoped Vercel public site key are configured. A fresh default-off Preview and later enabled end-to-end test remain pending.
