...
- Verify audio upload restrictions (allowed MIME types + max size caps). PR #48 merged as `6f655f23` and migration `20260903163658` is applied to staging and production. MP3, WAV, and M4A passed in Preview; live oversized and renamed non-audio rejection also passed. Production closeout is paused because a valid AIFF reached Storage but its direct PUT response remained open, leaving the browser at 100% before finalization. All three PR #50 Preview candidates reproduced the stall, including the one-shot timer at `c3e0f2e9` on 17 September. Rounded completion and timer resets remain unproven hypotheses. Use opt-in upload diagnostics to identify the actual failing boundary before changing recovery again. Verify recovery in staging and production before marking this item done.
